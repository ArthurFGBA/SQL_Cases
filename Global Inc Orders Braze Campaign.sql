...
(
    SELECT UPPER(SPLIT_PART(EXTERNAL_USER_ID, '_', 1)) AS COUNTRY,
           EXTERNAL_USER_ID,
           IFF(FILE_NAME ILIKE '%EnrollInControl%', 'CONTROL', 'TEST') AS CONTROL_GROUP,
           MIN(CAST(TIME AS timestamp)::DATE) AS ENTRY_DATE
    FROM CO_WRITABLE.GLOBAL_BRAZE_AIRFLOW
    WHERE  CAMPAIGN_NAME = 'BR_LOCAL_ONEOFF_PUSH_ALTADEMANDA_BELOHORIZONTE_22042024'
      AND COUNTRY = 'BR'
    GROUP BY 1,2,3 
),
ORDERS AS
(
    ...
    ON B.EXTERNAL_USER_ID = CONCAT(UPPER(O.COUNTRY),'_',O.STOREKEEPER_ID)
       AND O.CREATED_AT >= B.ENTRY_DATE --O.CREATED_AT::DATE '2024-01-18 16:00:00'
       AND O.CREATED_AT < B.ENTRY_DATE+10 -- '2024-01-18 18:30:00'
    GROUP BY 1,2,3,4
),
TOTALS AS 
(
